...
{  
  ...
  ros::Publisher pub_escape_command = n.advertise<std_msgs::Float32>("escape",10);
  ...
  std_msgs::Float32 escape_command;
  ...
  while (ros::ok() && kbd.get_key(new_event, pressed, k.code, k.modifiers)) {
    if (new_event || pressed) {
      ...
      if (pressed) { 
        ...
        if (k.code == 275) {
          if(steer_ang < 670.0){   //115 for manual keyboard 'digipot switch' control
	...
	pub_brk_command.publish(brk_command);
        pub_escape_command.publish(escape_command);
      }
